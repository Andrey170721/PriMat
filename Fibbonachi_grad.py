...
def gradient_descent(best_estimates):
    ...
    f = fib1
    n -= 2
    fn = (1 / np.sqrt(5)) * (((1 + np.sqrt(5)) / 2) ** n - ((1 - np.sqrt(5)) / 2) ** n)
    fn1 = (1 / np.sqrt(5)) * (((1 + np.sqrt(5)) / 2) ** (n + 1) - (((1 - np.sqrt(5)) / 2) ** (n + 1)))
    fn2 = (1 / np.sqrt(5)) * (((1 + np.sqrt(5)) / 2) ** (n + 2) - (((1 - np.sqrt(5)) / 2) ** (n + 2)))

    best_x, best_y = best_estimates
    descent_step = (fn / fn2) * 0.1
